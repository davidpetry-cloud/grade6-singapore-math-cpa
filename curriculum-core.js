...
function rounding(host){
  ...
  function render(rounded){
    ...
    svg.innerHTML=`
      ...
      <text x="60" y="92" font-family="Lexend" font-size="13" fill="#5B7189" text-anchor="middle">${cm(lo)}</text>
      <text x="640" y="92" font-family="Lexend" font-size="13" fill="#5B7189" text-anchor="middle">${cm(hi)}</text>
      <text x="350" y="106" font-family="Lexend" font-size="12" fill="#B87C1C" text-anchor="middle">midpoint ${cm(mid)}</text>
      <g class="mk" style="transform:translateX(${mx-350}px)">
        ...
        <text x="350" y="26" font-family="Lexend" font-size="13" fill="#fff" text-anchor="middle">${cm(rounded?target:n)}</text>
      </g>`;
...
function barModel(host){
  ...
  function bar(x,y,w,h,fill,label){
    return `<rect class="bmseg" x="${x}" y="${y}" width="${w}" height="${h}" rx="3" style="fill:${fill}" stroke="#fff" stroke-width="1.5"/>
      <text x="${x+w/2}" y="${y+h/2+5}" font-family="Lexend" font-size="14" fill="#fff" text-anchor="middle">${label}</text>`;
  }
  function draw(){
    ...
    if($m.value==="pw"){
      ...
      svg.innerHTML=`
        <text x="${X0+W/2}" y="26" font-family="Lexend" font-size="13" style="fill:var(--muted)" text-anchor="middle">whole = ${cm(w)}</text>
        ...
        <text x="${X0+W/2}" y="132" font-family="Lexend" font-size="15" style="fill:var(--ink-2)" text-anchor="middle">${cm(w)} − ${cm(p)} = ${cm(r)}</text>`;
      ...
      if(diffW>0)s+=`<path d="M${diffX} 158 L${diffX} 166 L${diffX+diffW} 166 L${diffX+diffW} 158" fill="none" style="stroke:var(--amber-dk)" stroke-width="1.5"/>
        <text x="${diffX+diffW/2}" y="182" font-family="Lexend" font-size="13" style="fill:var(--amber-dk)" text-anchor="middle">difference ${cm(Math.abs(b-a)*u)}</text>`;
      svg.innerHTML=s;
...
function decimalGrid(host){
  ...
  function grid(x,y,S,val,fill,label,second,fill2){
    ...
    if(label)s+=`<text x="${x+S/2}" y="${y+S+22}" font-family="Lexend" font-size="14" fill="${INK}" text-anchor="middle">${label}</text>`;
    return s;
  }
  ...
  function draw(){
    ...
    if(m==="show"){
      ...
      s+=`<text x="40" y="${30+S*0.62+52}" font-family="Lexend" font-size="13" fill="${MUT}">tenths = full columns · hundredths = single cells</text>`;
      svg.innerHTML=s;
    ...
    if(m==="mul"){
      ...
      s+=`<text x="${GX+S/2}" y="${GY+S+24}" font-family="Lexend" font-size="13" fill="${MUT}" text-anchor="middle">${(bc/10)} across</text>`;
      s+=`<text x="${GX-14}" y="${GY+S/2}" font-family="Lexend" font-size="13" fill="${MUT}" text-anchor="middle" transform="rotate(-90 ${GX-14} ${GY+S/2})">${(ac/10)} down</text>`;
      s+=`<text x="${GX+S+40}" y="${GY+70}" font-family="Lexend" font-size="14" fill="${INK}">overlap = ${cells} cells</text>`;
      s+=`<text x="${GX+S+40}" y="${GY+94}" font-family="Lexend" font-size="14" fill="${INK}">each cell = 0.01</text>`;
    ...
    if(m==="cmp"){
      svg.innerHTML=grid(70,26,S,Math.min(1,a),T,String(a))+grid(330,26,S,Math.min(1,b),A,String(b))+
        `<text x="350" y="${26+S+52}" font-family="Lexend" font-size="13" fill="${MUT}" text-anchor="middle">shade both, then look — no digit-counting required</text>`;
...
function rateLine(host){
  ...
  function scale(y,label,vals,unit,colour,dash){
    ...
    s+=`<text x="${X0-10}" y="${y+4}" font-family="Lexend" font-size="12" fill="${MUT}" text-anchor="end">${label}</text>`;
    vals.forEach((v,i)=>{
      ...
      s+=`<text x="${x}" y="${y+(dash?24:-15)}" font-family="Lexend" font-size="12" fill="${INK}" text-anchor="middle">${q(v,unit)}</text>`;
    });
    return s;
  }
