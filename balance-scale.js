...
function balanceScale(host){
  ...
  function pan(cx,cy,cups,units,tone){
    ...
    for(let i=0;i<cups;i++){
      s+=`<path d="M${x} ${y-18} L${x+20} ${y-18} L${x+16} ${y} L${x+4} ${y} Z" fill="${P}" stroke="#4A5C8C" stroke-width="1.5"/>
          <text x="${x+10}" y="${y-4}" font-family="Lexend" font-size="11" fill="#fff" text-anchor="middle">x</text>`;
    ...
    if(units>60)s+=`<text x="${cx}" y="${dy-22}" font-family="Lexend" font-size="12" fill="${MUT}" text-anchor="middle">+${units-60} more</text>`;
    return s;
  }
  ...
  function draw(){
    ...
    if(rel==="="&&broken)s+=`<text x="${cx}" y="58" font-family="Lexend" font-size="13" fill="${ROSE}" text-anchor="middle">no longer the sentence you started with</text>`;
    svg.innerHTML=s;
